...
__webpack_require__.r(__webpack_exports__);
/* eslint no-bitwise: 0 */
function isBitSet(_byte, bitPos) {
  return _byte & 1 << bitPos;
}
/**
...
function unpackBinaryFrame(byteArray, frameOffset, pixelsPerFrame) {
  ...
  for (var i = 0; i < pixelsPerFrame; i++) {
    ...
    var _byte2 = byteArray[bytePos + frameOffset]; // Bit position (0-7) within byte

    var bitPos = i % 8; // Check whether bit at bitpos is set

    pixelData[i] = isBitSet(_byte2, bitPos) ? 1 : 0;
  }
